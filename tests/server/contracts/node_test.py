...
async def test_node(contracts_with_tokens: Tuple[Contracts, Contracts, Contracts], tx_option):
    ...
    await c1.token_contract.approve(node_contract_address, Web3.to_wei(400, "ether"), option=tx_option)
    await c2.token_contract.approve(node_contract_address, Web3.to_wei(400, "ether"), option=tx_option)
    await c3.token_contract.approve(node_contract_address, Web3.to_wei(400, "ether"), option=tx_option)
    ...
    await c2.node_contract.pause(option=tx_option)

    assert (await c1.node_contract.available_nodes()) == 2
    assert (await c1.node_contract.total_nodes()) == 3

    await c2.node_contract.resume(option=tx_option)
